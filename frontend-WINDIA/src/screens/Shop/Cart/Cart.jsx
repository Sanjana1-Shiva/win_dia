      ...
      <div className="cart-page">
        ...
        <div className="container cart-body">
          <motion.div className="cart-empty" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p>Add some delicious things to get started!</p>
            <Link href="/shop" className="btn btn-primary">Explore Thins <FiArrowRight /></Link>
          </motion.div>
        </div>
      </div>
    );
              ...
              {cartItems.map((item) => (
                ...
                >
                  ...
                  <div className="item-quantity">
                    <button className="qty-btn" aria-label={`Decrease quantity of ${item.name}`} onClick={() => handleQuantityChange(item._id, -1, item.qty, item.countInStock)}><FiMinus /></button>
                    <span className="qty-value" aria-live="polite">{item.qty}</span>
                    <button className="qty-btn" aria-label={`Increase quantity of ${item.name}`} onClick={() => handleQuantityChange(item._id, 1, item.qty, item.countInStock)}><FiPlus /></button>
                  </div>
                  ...
                  <button className="item-remove" aria-label={`Remove ${item.name} from cart`} onClick={() => handleRemoveItem(item._id, item.name)}><FiTrash2 /></button>
                </motion.div>
              ))}
            </AnimatePresence>
